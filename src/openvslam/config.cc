...
config::config(const YAML::Node& yaml_node, const std::string& config_file_path)
    ...
    if (camera_->setup_type_ == camera::setup_type_t::Stereo || camera_->setup_type_ == camera::setup_type_t::RGBD) {
        // Ignore depths that are more than a fixed multiple of the baseline length
...
config::config(const YAML::Node& yaml_node1, const std::string& config_file_path1, const YAML::Node& yaml_node2, const std::string& config_file_path2, const std::string& jak_traj_path)
    ...
    try {
        switch (camera_model_type) {
            case camera::model_type_t::Perspective: {
                camera_ = new camera::perspective(yaml_node_);
                camera2_ = new camera::perspective(yaml_node2_);
                break;
            }
            case camera::model_type_t::Fisheye: {
                camera_ = new camera::fisheye(yaml_node_);
                camera2_ = new camera::fisheye(yaml_node2_);
                break;
            }
            case camera::model_type_t::Equirectangular: {
                throw;
